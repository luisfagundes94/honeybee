...
internal class PremiumViewModelTest {
    ...
    private val offers = MutableStateFlow<List<SubscriptionOffer>>(emptyList())
    private val isPurchasePending = MutableStateFlow(false)
    ...
    @BeforeEach
    fun setUp() {
        every { repository.status } returns status
        every { repository.offers } returns offers
        every { repository.isPurchasePending } returns isPurchasePending
    ...
    fun `offer updates should select annual plan and purchase should use domain offer token`() = runTest {
        // Given
        val monthly = SubscriptionOffer("monthly", SubscriptionPlan.MONTHLY, "$5", "monthly-token")
        val annual = SubscriptionOffer("annual", SubscriptionPlan.ANNUAL, "$40", "annual-token")

        // When
        offers.value = listOf(monthly, annual)
        ...
        viewModel.uiEffect.test {
            ...
            // Then
            assertEquals(PremiumUiEffect.LaunchPurchase(annual.offerToken), awaitItem())
        }
    }
